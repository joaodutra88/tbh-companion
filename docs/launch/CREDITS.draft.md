...
> DRAFT de lançamento. O TBH Companion é uma **fusão** de trabalho de outras pessoas da
> comunidade do TBH: Task Bar Hero. Esta página dá crédito explícito — quem, o quê, link e licença.
> João quer creditar de verdade quem ele "foi juntando coisa".
...
> ⚠️ **Correção de crédito pendente (importante):** o arquivo [`LICENSE`](../../LICENSE) na raiz do
> repo cita atualmente **`(c) lezards`**. Para bater **exatamente** com o copyright holder upstream,
> deve virar **`(c) EuSouOGiba (https://eusouogiba.com)`**. `lezards` é só o handle do GitHub; o
> detentor do copyright declarado no projeto original é **EuSouOGiba**. Trocar antes do lançamento.
